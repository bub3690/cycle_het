...
class SeverityDataset_cycle(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform: 
            image = self.transform(image).type(torch.float32)
            if self.train:
                image = self.weak_transform(image) # for teacher
                image2 = self.strong_transform(image) # for student 
        
        # 3채널
        if image.shape[0] != 3:
            image = image.expand(3,-1,-1)
            image2 = image2.expand(3,-1,-1)
            
        if self.global_regression:
            # global regression을 위한 레이블링. 모든 레이블의 합.
            global_label = torch.tensor([torch.sum(now_label)],dtype=torch.float32)
            return image,image2, now_label, now_label_float, global_label, filename, idx
            
        
        return image, image2, now_label, now_label_float, filename, idx
